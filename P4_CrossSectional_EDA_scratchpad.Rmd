...
##Add IMD for EW by LSOA
##Add inIMD for E by LSOA

#Scatterplot: income deprivation rank by EPCs
##consider doing a ggside for the IMD
...
```{r}
ggplot(sample[], aes(x = nn_CURRENT_ENERGY_EFFICIENCY, colour = nn_construction_age_band )) +
  ...
  facet_grid(rows = vars(n_builtForm))


#facetgrid: tenure x built form, region

#on running vs environmental: "CURRENT_ENERGY_EFFICIENCY", "ENVIRONMENT_IMPACT_CURRENT", "CO2_EMISS_CURR_PER_FLOOR_AREA","TOTAL_FLOOR_AREA"
## is the difference of  POTENTIAL - CURRENT, larger for ENERGY_EFFICIENCY  vs. ENVIRONMENT_IMPACT? Are people max out on the greedy metric?
## RQ: Is the cost environmental impact further from its potential than the running cost? 


#on rich and spatious:
#"CO2_EMISS_CURR_PER_FLOOR_AREA", "EXTENSION_COUNT","NUMBER_HABITABLE_ROOMS","NUMBER_HEATED_ROOMS","TOTAL_FLOOR_AREA"
#View(big_Dt_subset[by = "TRANSACTION_TYPE", .(n = .N)])
##RQ: Are the biggest homes, the most inefficient ones? Is there a difference by tenure? 
```
...
ggplot(big_Dt, aes(x=CURRENT_ENERGY_EFFICIENCY, y = regionName, fill=factor(..quantile..))) +
