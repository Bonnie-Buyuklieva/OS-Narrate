...
tmp$n_deltaDays <- ifelse( tmp$firstEntry == T, 
                       0 , as.numeric(tmp$INSPECTION_DATE, tmp$n_delta) )  
...
for (entry in unique(tmp[,dupcol])) {
  ...
  if(row > 1){ #row one is always the first entry
    ...
    #tmp$n_dER[which(tmp$LMK_KEY %in% tmp_LMKs[row])] <- tmp_entries$n_dER[row]
    #tmp$n_dEE[which(tmp$LMK_KEY %in% tmp_LMKs[row])] <- tmp_entries$n_dEE[row]
  }
}
```
