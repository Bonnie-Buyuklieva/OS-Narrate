...
for (LA in folders) {
  ...
  if( counter %% 20 == 0){print(LA)} 
    ...
    repeat_nUPRN <- duplicated(certificates$n_UPRN)|duplicated(certificates$n_UPRN, fromLast = T) 
    NA_nUPRN <- which(is.na(certificates$n_UPRN))# remove nas that slip through as duplicates
    repeat_nUPRN <- setdiff(which(repeat_nUPRN),NA_nUPRN)
 
    
    dup_idx <- union(which(repeat_str),repeat_nUPRN)
    ...
    #### length(unique(dict$str_concat)) -> sometimes you can have m
    ...
    View(duplicated_certificates[order(n_UPRN, str_concat), c('str_concat','n_UPRN','INSPECTION_DATE')])

    
    #issue:
    certificates[n_UPRN == '10009711003', c('str_concat','n_UPRN')]
    duplicated_certificates[n_UPRN == '10009711003', c('str_concat','n_UPRN')]
    duplicated(certificates[n_UPRN == '10009711003',c('str_concat','n_UPRN')]$n_UPRN)|duplicated(certificates[n_UPRN == '10009711003',c('str_concat','n_UPRN')]$n_UPRN, fromLast = T)
