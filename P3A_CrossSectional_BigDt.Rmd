...
big_Dt_CSsubset_N6M7[ , n_bIoDDecil_income_grp := cut(n_gIoDDecil_barriers,
                            breaks = c(0, 3, 7, Inf),
...
NS_dt <- big_Dt_CSsubset_N6M7[ n_CURRENT_ENERGY_EFFICIENCY < 101,] # drops = 3 094 
NS_dt <- NS_dt[ n_POTENTIAL_ENERGY_EFFICIENCY < 101,] # drops a further = 80 589

maxthreshold <- round(quantile(big_Dt_CSsubset_N6M7$TOTAL_FLOOR_AREA, 0.99, na.rm = T),0) 
minthreshold <- round(quantile(big_Dt_CSsubset_N6M7$TOTAL_FLOOR_AREA, 0.01, na.rm = T),0) 
...
sample <- lengths(NS_dt )[1] # 16 326 599
sample


save( NS_dt , file = "../Outputs/WIP_crossSectional/NS_subset_N16m326599.RData")
```
