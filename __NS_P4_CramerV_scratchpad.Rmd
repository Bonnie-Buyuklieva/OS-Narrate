...
data[,ssimp_nn_construction_age_band := nn_construction_age_band]
...
data[ ,x_PropT3_Tenure3_Age8:= paste(ssimp_nn_construction_age_band,n_propTyp,n_tenure)]
data[ ,x_PropT3_Age8:= paste(n_propTyp,ssimp_nn_construction_age_band)]


data[ ,x_PropT3_Tenure3:= paste(n_propTyp,n_tenure)]
data[ ,X_Tenure3_Age8:= paste(n_tenure,ssimp_nn_construction_age_band)]

data[ ,x_Tenure3_habitable_rooms_grp10:= paste(n_tenure, n_habitable_rooms_grp)]
data[ ,x_Tenure3_habitable_rooms_grp10_PropT3:= paste(n_tenure, n_propTyp, n_habitable_rooms_grp)]

data[,n_CEE_binary := n_CEErel]
fct <- c("C & C+","C & C+","C-")
levels(data$n_CEE_binary) <- fct
data[,n_CEE_binary := factor(n_CEE_binary, levels = unique(fct), ordered = T)]

data[ ,x_Tenure3_CEE_binary:= paste(n_tenure, n_CEE_binary)]
...
ggplot() + geom_histogram(data = data[RGN11NM == 'London'], 
  ...
  #scale_y_continuous(name = 'Count of EPCs' ,breaks=seq(0,100000,10000))+
  theme(legend.position="none")+
  labs(title = 'Current Energy Efficiency in London',
         tag = paste0("N = ", format(lengths(data)[1], big.mark=" ", trim=TRUE)) ) 
...
for (col in interest_factors_cols) {
  #DROPPING C - LIKELY TO GIVE HIGHER CORRELATION, easier to plot
  #make Paneldata
  tmp_data <- data[ n_CEErel != 'C' ,  .(n = .N), by = c(col, measure, 'x_PropT3_Tenure3_Age8') ]
  tmp_panelsheet <- tmp_data
  tmp_panelsheet$topic <- col
  names(tmp_panelsheet) <- c('subgroup', 'mobility','year','n', 'topic')
  panelsheet <- rbind(panelsheet, tmp_panelsheet)
