...
subsetCols <- c("LMK_KEY","INSPECTION_DATE","LODGEMENT_DATE",
            ...
            "ADDRESS", "POSTCODE",
            "UPRN","UPRN_SOURCE","LOCAL_AUTHORITY", "LOCAL_AUTHORITY_LABEL","POSTTOWN","CONSTITUENCY_LABEL")
```
...
for (LA in folders) {
  ...
  certificates$folder <- LA 
  print(paste('load time:', Sys.time() - start.time)) #1.84
  
  
  
  
  #speed by reading less?
  start.time <- Sys.time()
    certificates_fast <- read_csv(paste0("../Data/all-domestic-certificates/",LA,"/certificates.csv"), 
    col_types = cols_only(
                          ...
                          COUNTY = '?') )
    certificates_fast$folder <- LA #folder info
   print(paste('load time:', Sys.time() - start.time)) #load time: 1.49
   certificates <- certificates_fast
  ...
  tmpdups1$naUPRN <- sum(is.na(certificates$UPRN))
  uprn_dups <- rbind(uprn_dups,tmpdups)
  ...
  tmpdups2 <- certificates[duplicated(certificates$UPRN) | duplicated(certificates$UPRN, fromLast = TRUE),]
  print(paste('time with unlist:', Sys.time() - start.time)) # "time with unlist: 3.45"
  ...
  #no unlisting
  #with ordering - very expensive
  start.time <- Sys.time()
  certificates <- certificates[order(certificates$LMK_KEY),] #order certificates as the LUT
  certificates$udbc_UPRN <- ubdc_link$ubdc_uprn[match(certificates$LMK_KEY, ubdc_link$lmk_key)]
  tmpdups2_1 <- certificates[duplicated(certificates$UPRN) | duplicated(certificates$UPRN, fromLast = TRUE),]
  print(paste('time without unlist:', Sys.time() - start.time)) #"time without unlist: 4.86 (ubdc_link-unordered) / 4.43 (ubdc_link-ordered)"
...
#write.csv(folders_meta, '../Outputs/  .csv')
```
