...
LAD_row_tally_after_replace_by_LUT <- function( tmp = tmp, EPC_col_topic = 'TENURE', harm_LUT = LUT_tenure, reg = region, LAD = LA){

  ##LUT_colum <- LUT[match(tmp$TENURE, LUT$tenure_factors),"harmonisation"]
  #LUT col 1 is all possible factors 
  colID <- which(names(tmp) == EPC_col_topic)
  LUT_colum <- harm_LUT[match(tmp[,colID], harm_LUT[,1]),"harmonisation"]
...
```{r LUT_loop }
  LUT_tenure <- data.table(LUT_tenure)
  ...
  for (region in cross_data) {
    ...
    for (LA in unique(tmp$LAD11NM)) {
      ...
      row <- LAD_row_tally_after_replace_by_LUT( EPC_col_topic = 'TENURE', harm_LUT = LUT_tenure, reg = region, LAD = LA)
      LAD_table <- rbind(LAD_table, t(row))
    }
