...
#get loopUp
LUT <- read_excel("../Data/OS-N_EPC_v10_LUT+dictionaries.xlsx", sheet = "TENURE",range = cell_cols("B:C"))
```
...
for (region in cross_data) {
  ...
  #N1: Tenure
  EPC_col_topic <- 'TENURE'
  LUT_col_factors <- 'tenure_factors'
loadRData <- function(){
  ...
  LUT_colum <- LUT[match(tmp[,EPC_col_topic], LUT[,LUT_col_factors]),"harmonisation"]
