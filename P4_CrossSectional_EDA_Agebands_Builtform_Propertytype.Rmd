...
```{r}
#Adding the 
#big_Dt <- loadRData("../Outputs/WIP_crossSectional/ONSUD_crossSectional_subset_N16744771.RData")

plot_dt <- big_Dt[ , by = c('BUILT_FORM', 'PROPERTY_TYPE'),  .(n = .N)]
#plot_dt$percent <-  plot_dt$n/ sum(plot_dt$n)
plot_dt$nK <- plot_dt$n/1000
  
ggplot(plot_dt, aes(x= BUILT_FORM, y=nK))+
  geom_bar(stat='identity')+
...
big_Dt[, n_BFPT := paste(n_builtForm, PROPERTY_TYPE)]

set.seed(100)
a <- big_Dt[, .SD[sample(.N, 5)], by = n_BFPT]
...
```{r}
#to do plot tenure by building type 
plot_dt <- big_Dt[ , by = c('BUILT_FORM', 'PROPERTY_TYPE', 'n_tenure'),  .(n = .N)]
plot_dt[ ,nTenure := sum(n), by=n_tenure]
plot_dt[ ,wTenurePercent := round(n/nTenure*100,0)]



ggplot(plot_dt, aes(x= BUILT_FORM, y=wTenurePercent))+
  geom_bar(stat='identity')+
  facet_grid(PROPERTY_TYPE ~ n_tenure) +
  coord_flip()+
  theme_minimal()+
  labs( x = '', y = 'Properties as % of tenure group', )+
  labs(subtitle = paste( 'Proportions of tenure group:' ,c( round(unique(plot_dt$nTenure)/sum(plot_dt$n)*100,0))) )
